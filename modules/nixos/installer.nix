{
  ...
  systemd.services.inception = {
    ...
    script = with pkgs; ''
      sleep 60
      ${config.system.build.nixos-install}/bin/nix --experimental-features "nix-command flakes" run github:nix-community/disko -- --mode disko ${toString self}/modules/disko/disko-config.nix
